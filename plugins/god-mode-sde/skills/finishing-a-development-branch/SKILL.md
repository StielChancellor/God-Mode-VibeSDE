...
- Worktree under `.worktrees/`, `worktrees/`, or `~/.config/god-mode/worktrees/` → we created it, we clean up:
  ```bash
  MAIN_ROOT=$(git -C "$(git rev-parse --git-common-dir)/.." rev-parse --show-toplevel)
  cd "$MAIN_ROOT"
