...
Sets up isolation BEFORE **Stage 6 (Build)** — required by `executing-plans`, `subagent-driven-development`, and `dispatching-parallel-agents` so parallel coding agents don't collide. Cleanup is handled by `finishing-a-development-branch`. Priority: **user > skills > default**; `_shared/god-mode-principles.md` apply.
...
3. Existing global dir: `~/.config/god-mode/worktrees/<project>`.
...
# project-local: path="$LOCATION/$BRANCH_NAME"  |  global: path="~/.config/god-mode/worktrees/$project/$BRANCH_NAME"
