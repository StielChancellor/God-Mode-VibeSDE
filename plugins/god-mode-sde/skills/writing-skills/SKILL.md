...
Meta-skill — it builds the tools the pipeline runs on rather than sitting at one stage. Use it whenever you extend the God-Mode plugin (a new methodology skill, a domain skill, a role's playbook). Priority: **user > skills > default**; `_shared/god-mode-principles.md` apply. New skills must respect the same voice: crisp, imperative, no AI-slop hedging.
