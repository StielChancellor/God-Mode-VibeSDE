// Shared helpers for God-Mode SDE guardrail hooks. Cross-platform (Node, no deps).
...
export const isAdvisory = () =>
  (process.env.GODMODE_GUARDRAILS || '').toLowerCase() === 'advisory';
...
export function hardBlock(event, reason) {
  if (isAdvisory()) { emit(event, { additionalContext: '[GODMODE advisory — would BLOCK] ' + reason }); process.exit(0); }
