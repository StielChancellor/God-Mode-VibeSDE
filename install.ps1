# God-Mode SDE — one-shot installer for Windows / Claude Desktop users.
#
...
# vibe-fde marketplace, installs god-mode-sde, and leaves it OFF by default (token-safe).
# Re-running it is safe (idempotent).

$ErrorActionPreference = 'Continue'
function Say($m, $c = 'White') { Write-Host $m -ForegroundColor $c }

Say "`n=== God-Mode SDE installer ===`n" 'Cyan'
...
try { Claude plugin marketplace add 'StielChancellor/God-Mode-VibeSDE' } catch { Say "  (marketplace already added — ok)" 'DarkGray' }

Say "Installing plugin (god-mode-sde@vibe-fde)..."
try { Claude plugin install 'god-mode-sde@vibe-fde' } catch { Say "  (plugin already installed — ok)" 'DarkGray' }

# 3) Off by default — keeps non-dev projects at 0 extra tokens.
Say "Setting it OFF by default (token-safe)..."
try { Claude plugin disable 'god-mode-sde@vibe-fde' --scope user } catch {}

# 4) Show status.
Say "`nCurrent status:" 'Cyan'
Claude plugin list

Say "`n[OK] God-Mode SDE is installed and OFF by default (saves ~9.5k tokens/session where you don't need it)." 'Green'
Say "To use it on a project: cd into the project, run the line below, then restart Claude:" 'Green'
Say "    claude plugin enable god-mode-sde@vibe-fde --scope project" 'White'
